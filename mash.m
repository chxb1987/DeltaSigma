...
[v,xn_v,xnmax_v,y_v] = simulateDSM(7.*(v_tmp-y),H,8); v_12 = zeros(numel(v),12);
for i=1:numel(v)
    if (v(i) == -7)
...
v_tmp_1 = zeros(numel(v),12); v_1 = zeros(numel(v),12); v_2 = zeros(numel(v),12); v_mash = zeros(numel(v),12);

v_tmp_1(1,:) = sgn_xtnd(v_12(1,:)); 
v_1(1,:) = bin_add(sgn_xtnd(sgn_xtnd(v_12(1,:))),zeros(1,12));
v_2(1,:) = bin_add(sgn_xtnd(v_1(1,:)),zeros(1,12));
v_mash(1,:) = bin_add(two_cmpl(v_2(1,:)),v_tmp_1(1,:));

v_tmp_1(2,:) = bin_add(sgn_xtnd(v_tmp12(2,:)),bin_add(two_cmpl(bin_add(sgn_xtnd(sgn_xtnd(sgn_xtnd(v_tmp12(1,:)))),sgn_xtnd(v_tmp12(1,:)))),zeros(1,12)));
v_1(2,:) = bin_add(sgn_xtnd(sgn_xtnd(v_12(2,:))),two_cmpl(sgn_xtnd(sgn_xtnd(v_12(1,:)))));
v_2(2,:) = bin_add(sgn_xtnd(v_1(2,:)),two_cmpl(sgn_xtnd(v_1(1,:))));
v_mash(2,:) = bin_add(two_cmpl(v_2(2,:)),v_tmp_1(2,:));

for i=3:numel(v)
v_tmp_1(i,:) = bin_add(sgn_xtnd(v_tmp12(i,:)),bin_add(two_cmpl(bin_add(sgn_xtnd(sgn_xtnd(sgn_xtnd(v_tmp12(i-1,:)))),sgn_xtnd(v_tmp12(i-1,:)))),sgn_xtnd(sgn_xtnd(v_tmp12(i-2,:)))));
v_1(i,:) = bin_add(sgn_xtnd(sgn_xtnd(v_12(i,:))),two_cmpl(sgn_xtnd(sgn_xtnd(v_12(i-1,:)))));
v_2(i,:) = bin_add(sgn_xtnd(v_1(i,:)),two_cmpl(sgn_xtnd(v_1(i-1,:))));
v_mash(i,:) = bin_add(two_cmpl(v_2(i,:)),v_tmp_1(i,:));
end
...
v_mash_a = zeros(1,numel(v));
for i=1:numel(v)
v_mash_a(i) = -v_mash(i,1) + bi2de([0 v_mash(i,2:12)],'left-msb')/(2^11);
end
% v_mash_a = (64).*v_mash_a;
% figure;
% plot(v_mash_a); hold on; plot(2.*u); grid on;
...
snr_mash = calculateSNR(spec_mash(3:fB+1),ftest-2);

legend(sprintf('2-bit Quantizer SNR = %4.1fdB @ OSR = %d',snr_v,OSR),sprintf('1-bit Simulation SNR = %4.1fdB @ OSR = %d',snr,OSR),sprintf('MASH SNR = %4.1fdB @ OSR = %d',snr_mash,OSR));
