...
v = ds_quantize(3.*y,4);
v_dig = 0.5 + 0.5.*v;
v1_dig = xor(v_dig,[1 1-v_dig(1:numel(v)-1)]);
v2_dig = xor(v1_dig,[1 1-v1_dig(1:numel(v)-1)]);
v1a = v - [-3 v(1:numel(v)-1)]; 
v2a = v1a - [-4 v1a(1:numel(v)-1)]; 
v1a = v1a./4; v2a = v2a./8;
v_lsli = v_tmp_1 + v2a;
v_lsli_da = 2.*v2_dig - 1;
% v = 0.5 + 0.5.*v;
...
vw_dig = 0.5 + 0.5.*vw;
v1w_dig = xor(vw_dig,[1 1-vw_dig(1:numel(v)-1)]);
v2w_dig = xor(v1w_dig,[1 1-v1w_dig(1:numel(v)-1)]);
v1aw = vw - [-3 vw(1:numel(vw)-1)]; 
v2aw = v1aw - [-4 v1aw(1:numel(v1aw)-1)]; 
v1aw = v1aw./4; v2aw = v2aw./8;
vw_lsli = vw_tmp_1 + v2aw;
vw_lsli_da = 2.*v2w_dig - 1;
...
hwv_lsli_da = fftshift(abs(fft(v2_dig'.*hanning(N+1))))./(N+1); hwvw_lsli_da = fftshift(abs(fft(v2w_dig'.*hanning(N+1))))./(N+1);
...
plot(log10(fx),20.*log10(hwv_tmp_1));  
...
plot(log10(fx),20.*log10(hwv_tmp)); hold on; plot(log10(fx),20.*log10(hwv_lsli)); hold on; grid on;
snrv_lsli_da = 10*log10(sum(hwv_lsli_da(fint).^2)/sum(hwvw_lsli_da(fint).^2));
% plot(log10(fx),20.*log10(hwv_lsli_da)); 
% legend(sprintf('snrvtmp = %f',snrv_tmp),sprintf('snrvlsli = %f',snrv_lsli),sprintf('snrvlslida = %f',snrv_lsli_da));
